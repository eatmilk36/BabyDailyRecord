...
使用者回報「點下去超慢」。量測結果證明**不是程式問題**：

| 量測 | 值 |
|---|---|
| 啟動系統「設定」App（與本專案無關） | **8568ms** |
| 主機 CPU | **94%**（`rider64` 累積 11194 CPU 秒 + DPA backend + testhost） |
| 主機記憶體 | 25.8 / 31.3 GB |
| AVD 記憶體 | **2048MB**（Android 17 + Play 服務 + Expo Go 太少） |
| 螢幕 | 1344×2992 @ 480dpi，`hw.gpu.mode=auto` |

一個系統 App 要 8.5 秒（健康機器約 1 秒）就足以證明瓶頸在模擬器層。在 CPU 94% 的情況下**無法有意義地量測 APP 自己的效能**。

**已做**：AVD `hw.ramSize` 2048 → **4096**、`vm.heapSize` 256 → **512**，冷開機生效（`/proc/meminfo` 確認 MemTotal ≈ 4GB）。

**仍待使用者處理**：開發時關掉 Rider（那是最大的 CPU 來源）。若仍慢，改用像素較少的裝置設定檔（Pixel 6，1080×2400，合成量少一半）。

註：模擬器曾在主機 CPU 94% 時直接掛掉。

### v2 修正的實機驗證（已通過）

- 首頁底部「兩個一起」那排捲到底後完整顯示，與 tab bar 有留白
- 彈窗顯示 `✓ 已儲存 05:09`、「完成」是實色主要按鈕、刪除降級為小字連結
- 紀錄頁日期導覽：`今天` 時右箭頭停用；翻到 `昨天` 後箭頭啟用、出現「今天」跳回、分寶總結歸零、空狀態顯示「這天沒有紀錄」
- **關鍵**：換日期時 `useEventsForDay` 的查詢真的重跑（昨天顯示 0 筆而非靜默沿用今天資料）—— deps 有正確傳入
...
## 11. 親餵超時的本地通知（已實作，補上規劃時漏掉的東西）

第 2.4 節原本寫「超時提醒」只做 APP 內的警示橫幅，推播留到第二階段。
實作後回頭看，這個切分是錯的：**你忘記按結束的時候，正是沒在看 APP 的時候。**
只有 APP 內橫幅的提醒，等於在你看得到的時候才提醒你你看得到的事。

### 為什麼可以現在就做（不需要伺服器）

推播分兩種，規劃時我把它們混為一談了：

| | 誰排程 | 需要伺服器 | Expo Go 可用 |
|---|---|---|---|
| **本地通知** | 手機自己 | ❌ 不需要 | ✅ 可以 |
| 遠端推播 | 你的後端 → FCM | ✅ 需要 | ❌ Android 自 SDK 53 起不行 |

超時提醒的資料（計時什麼時候開始）**全都在這台手機上**，所以屬於本地通知。
它跟同步功能沒有任何依賴關係，不該一起被延到第二階段。

### 實作

`lib/notifications.ts`，三個函式：

- `ensureNotificationPermission()` — 要權限
- `scheduleNursingOverdue(id, minutes, name)` — 排 60 分鐘後的提醒
- `cancelTimerNotification(id)` — 按結束時取消

**通知 id 直接用事件 id。** 這樣不需要另外存一份「事件 → 通知」的對應表；
少一份狀態就少一個會不同步的地方。同時哺餵沒有單一事件 id，所以用 `sessionId`
（只排一則 —— 兩則同時響是噪音，而內容一模一樣）。

### 兩個踩到的細節

1. **Android 13+ 必須先建立通知頻道，才能請求權限。** 順序反了權限對話框根本不會出現，
   而且不會報錯 —— 只是靜靜地什麼都沒發生。所以 `ensureChannel()` 在
   `requestPermissionsAsync()` 之前。
2. **`expo-notifications` 預設在 APP 前景時不顯示通知。** 要 `setNotificationHandler`
   明確開啟 `shouldShowBanner`，否則你正開著 APP 在別的頁面時提醒會被吃掉。

### 一開始寫錯的地方

同時哺餵的通知 id 我第一版寫成 `tandem-${Date.now()}`。排得出來、但**永遠取消不掉** ——
結束時手上沒有那個時間戳。改用 `sessionId` 才對稱。

### 權限被拒絕不擋任何事

`scheduleNursingOverdue` 回傳 `boolean` 但呼叫端用 `void` 忽略它。
沒有通知權限只是少了鎖屏提醒，記錄功能完全不受影響 —— 不該為此擋住「開始親餵」。
設定頁多了「測試通知權限」讓你確認狀態。

### 還沒驗證的部分

`expo export` 與 `tsc` 都過了，但**通知實際會不會響只能在你手機上測**。
特別是小米／華為／OPPO 的省電機制會延遲或直接吃掉排程通知 —— 設定頁的說明有寫怎麼排除。
